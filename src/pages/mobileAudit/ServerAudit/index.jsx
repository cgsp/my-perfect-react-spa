...
@Form.create({})
@observer
export default class ServerAudit extends Component {
    state={
    ...
    handleConfirmOk = () =>{
        this.ServerReject.props.form.validateFieldsAndScroll((errors, values) => {
            console.log(values)
        ...
        // TODO: 调接口提示 和上面的接口一样 但是入参有一点难取出而已
    }
    ...
    render() {
        ...
        return (
            <div className="server-top-container">
                <div className="server-title">服务端接入审批(移动&网站应用)</div>
                <CommonSearch mark = 'server' currentPage = {currentPage}/>
                <Table
                    className="table"
                    columns={columns}
                    dataSource={dataList}
                ...
                >
                    <CommonReject
                        wrappedComponentRef={ref => (this.ServerReject = ref)}
                    />
                </Modal>
            </div>
        )
